...
class Date
{
	...
	public function jdate($format, $timestamp = '', $none = '', $time_zone = 'Asia/Tehran', $tr_num = 'fa')
	{
		...
		$ts = $T_sec + (($timestamp === '') ? time() : $this->CI->date->tr_num($timestamp));
		$date = explode('_', date('H_i_j_n_O_P_s_w_Y', $ts));
		list($j_y, $j_m, $j_d) = $this->CI->date->gregorian_to_jalali($date[8], $date[3], $date[2]);
		...
		return ($tr_num != 'en') ? $this->CI->date->tr_num($out, 'fa', '.') : $out;
	}
	...
	{
		list($gy, $gm, $gd) = explode('_', $this->CI->date->tr_num($gy . '_' . $gm . '_' . $gd));/* <= Extra :اين سطر ، جزء تابع اصلي نيست */
